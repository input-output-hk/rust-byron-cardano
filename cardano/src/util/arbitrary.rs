...
impl<A> Wrapper<A> { pub fn unwrap(self) -> A { self.0 } }
impl<A> Deref for Wrapper<A> {
    type Target = A;
    fn deref(&self) -> &Self::Target { &self.0 }
}
impl<A> AsRef<A> for Wrapper<A> { fn as_ref(&self) -> &A   { &self.0    } }
impl<A> From<A>  for Wrapper<A> { fn from(a: A)    -> Self { Wrapper(a) } }

impl Arbitrary for Wrapper<config::ProtocolMagic> {
    fn arbitrary<G: Gen>(g: &mut G) -> Self {
        Wrapper(config::ProtocolMagic::from(<u32 as Arbitrary>::arbitrary(g)))
    }
}
...
impl Arbitrary for Wrapper<hdwallet::Seed> {
    fn arbitrary<G: Gen>(g: &mut G) -> Self {
        let vec : Vec<u8> = ::std::iter::repeat_with(|| Arbitrary::arbitrary(g))
            ...
            .collect();
        Wrapper(
            hdwallet::Seed::from_slice(&vec).unwrap()
        )
    }
}
