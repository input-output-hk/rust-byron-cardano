...
mnemonicLength s = go s.wordList
  where
  go arr = case uncons arr of
    Just { head: x, tail: xs } ->
        if x == "" then go xs else 1 + (go xs)
    Nothing -> 0

invalidMnemonicWord :: Mnemonic -> Maybe String
invalidMnemonicWord m = find search m.wordList
  where
