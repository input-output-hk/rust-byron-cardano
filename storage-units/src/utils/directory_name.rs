...
use std::{ops::Deref, error, fmt, str::{FromStr}};
...
impl fmt::Display for DirectoryNameError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            DirectoryNameError::InvalidCharacterAtIndex(index) => {
                write!(f, "Invalid directory name: contains an invalid character at index {}", index)
            },
            DirectoryNameError::UnsupportedCharacters(oss) => {
                write!(f, "Invalid directory name: contains invalid characters {:?}", oss)
            }
        }
    }
}
...
mod test {
    ...
    fn invalid_character_in_directory_name() {
        ...
        assert_eq!(DirectoryName::new("directory\\3".into()), Err(DirectoryNameError::InvalidCharacterAtIndex(9)));
    }
