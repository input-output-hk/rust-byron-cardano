...
impl From<io::Error> for InboundError {
    fn from(e: io::Error) -> Self { InboundError::IoError(e) }
}
impl From<nt::DecodeEventError> for InboundError {
    fn from(e: nt::DecodeEventError) -> Self { InboundError::EventParsingError(e) }
}
...
impl<T> InboundStream<T> {
    ...
    fn process_event(&mut self, event: nt::Event) -> Result<Inbound, InboundError>
    {
        match Message::from_nt_event(event) {
            ...
            Message::CloseConnection(lwcid) => {
                self.process_close_light_connection(lwcid)
            },
            Message::CloseEndPoint(lwcid)   => unimplemented!("Close end point not implemented ({:?}", lwcid),
            Message::CloseSocket(lwcid)     => unimplemented!("Close socket command not implemented {:?}", lwcid),
            Message::ProbeSocket(lwcid)     => {
                unimplemented!("Probe socket command not implemented {:?}", lwcid)
            },
            Message::ProbeSocketAck(lwcid)  => unimplemented!("Probe socket ack not implemented {:?}", lwcid),
            Message::CreateNodeId(lwcid, node_id) => {
                self.process_create_node_id(lwcid, node_id)
            },
    ...
    fn process_new_light_connection(&mut self, lwcid: nt::LightWeightConnectionId)
        -> Result<Inbound, InboundError>
    {
        ...
        if state.server_handles.contains_key(&lwcid) {
            ...
            state.server_handles.insert(lwcid, light_weight_connection_state);
            Ok(Inbound::NewConnection(lwcid))
        }
    }

    fn process_close_light_connection(&mut self, lwcid: nt::LightWeightConnectionId)
        -> Result<Inbound, InboundError>
    {
        let mut state = self.state.lock().unwrap();
            ...
            Some(light_state) => {
                if let Some(ref node_id) = &light_state.node {
                    if let Some(client_connection_id) = state.map_to_client.remove(node_id) {
                        if let Some(client_connection) = state.client_handles.get_mut(&client_connection_id) {
    ...
    fn process_create_node_id(&mut self, lwcid: nt::LightWeightConnectionId, node_id: NodeId)
        -> Result<Inbound, InboundError>
    {
        let mut state = self.state.lock().unwrap();
    ...
    fn process_ack_node_id(&mut self, lwcid: nt::LightWeightConnectionId, node_id: NodeId)
        -> Result<Inbound, InboundError>
    {
        let mut state = self.state.lock().unwrap();
        match state.server_handles.get_mut(&lwcid) {
            None => { return Err(InboundError::RemoteLightConnectionIdUnknown(lwcid)); },
            Some(ref mut light_state) => {
                light_state.node = Some(node_id);
            }
        }
        let client = state.client_handles.iter().find(|&(_, v)|
            v.node == Some(node_id)
        ).map(|(z, _)| *z);
