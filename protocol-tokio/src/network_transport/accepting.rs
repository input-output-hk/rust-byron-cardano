        ...
        loop {
            match self.state {
                AcceptingState::AwaitHandshake => {
                    ...
                    } else { return Err(AcceptingError::AlreadyConnected); };
                },
                AcceptingState::SendingResponse(succeed) => {
                    let done = if let Some(ref mut inner) = &mut self.inner {
                        debug!("sending response ({} out of {} bytes)", self.response.position(), self.response.get_ref().len());
                    ...
                    if done {
                        if succeed {
                            ...
                            return Err(AcceptingError::ConnectionFailed(ResponseCode::UnsupportedVersion))
                        }
                    }
                }
            }
...
impl fmt::Display for AcceptingError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            AcceptingError::AlreadyConnected => write!(f, "The connecting object was already connected and should have not been reused"),
        }
    }
}
